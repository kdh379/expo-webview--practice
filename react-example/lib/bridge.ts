...
export const bridge = {
  ...
  bluetooth: {
    ...
    onStatusChange: (callback: BridgeCallback<BluetoothStatusPayload>): BridgeCleanup => {
      if (typeof window === 'undefined') return () => {};

      const callbackId = generateId();
      callbackManager.set(callbackId, callback);

      sendBridgeMessage('REGISTER_BLUETOOTH_CALLBACK', { callbackId })
        .catch((error) => {
      ...
      return () => {
        callbackManager.delete(callbackId);
        sendBridgeMessage('UNREGISTER_BLUETOOTH_CALLBACK', { callbackId })
          .catch(console.error);
      };
    },
  },
